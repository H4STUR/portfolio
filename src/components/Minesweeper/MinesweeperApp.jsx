...
const MinesweeperApp = ({ id, title, onClose, position }) => {
  ...
  const handleCellClick = (row, col) => {
    ...
    if (!gameStarted) {
      ...
      if (cells[row][col].isMine) {
        // If it is a mine, only reveal the mine and set game over
      ...
      // If it's not a mine, proceed with revealing cells as usual
  ...
  useEffect(() => {
    const newSize = { width: rows * CELL_SIZE + rows * 2, height: cols * CELL_SIZE + cols + 200 };
    setSize(newSize);
    resetGame();
  }, [difficulty]);
  ...
  return (
    ...
    >
      ...
      {/* Game Grid */}
      <div className="minesweeper-grid" onMouseUp={handleMouseUp}>
